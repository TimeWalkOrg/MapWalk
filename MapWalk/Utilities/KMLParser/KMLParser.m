...
- (void)applyToOverlayPathRenderer:(MKOverlayPathRenderer *)renderer {
    if ([renderer.overlay isMemberOfClass:[MKPolygon class]]) {
        renderer.lineWidth = 30.0f;//strokeWidth;
    }
    else if ([renderer.overlay isMemberOfClass:[MKPolyline class]]) {
        renderer.lineWidth = 80.0f;//strokeWidth;
    }
    else {
        renderer.lineWidth = 30.0f;//strokeWidth;
    }
...
- (MKOverlayPathRenderer *)createOverlayPathRenderer:(MKShape *)shape {
    ...
    KMLConstantWidthPolygonRenderer *overlayPathView = [[KMLConstantWidthPolygonRenderer alloc] initWithPolygon: polygon];
...
//    return polyLine;
    ...
    MKPolyline *polyline = (MKPolyline *)shape;
    //MKPolygonRenderer *polyPath = [[MKPolygonRenderer alloc] initWithPolygon:(MKPolygon *)shape];
    
    KMLConstantWidthPolylineRenderer *overlayPathView = [[KMLConstantWidthPolylineRenderer alloc] initWithPolyline:polyline];
    return overlayPathView;
}
...
- (MKAnnotationView *)annotationView {
    if (!annotationView) {
        ...
        if (annotation) {
            ...
            //pin.image = [UIImage systemImageNamed: @"bubble.left.fill"];
            UIImage *pinImage = [UIImage systemImageNamed:@"bubble.left.fill"];
            pin.image = [pinImage imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
            //pin.image = [UIImage imageNamed:@"ic_bubble"]; // Set your custom image here
            annotationView = pin;
        }
    }
